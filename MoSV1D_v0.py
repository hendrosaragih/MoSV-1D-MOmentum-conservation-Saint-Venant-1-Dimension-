...
class Window(tk.Frame):
	...
	def Run(self):
		...
		# plt.ylabel("eta (m)",fontsize=14)
		self.ax = plt.title("MOmentum conservation Saint-Venant 1 Dimension")
		...
		self.ani = animation.FuncAnimation(self.fig, animate, frames=np.arange(1, self.eta_len, 10), interval=25)
	
	def on_click(self):
	    '''the button is a start, pause and unpause button all in one
	    this method sorts out which of those actions to take'''
	    if len(self.E0.get())==0 or len(self.E1.get())==0 or len(self.E2.get())==0 or len(self.E3.get())==0 or len(self.E4.get())==0 or len(self.E5.get())==0 or len(self.E6.get())==0 or len(self.E7.get())==0 or len(self.E8.get())==0 or len(self.E9.get())==0 or len(self.E10.get())==0 or len(self.E11.get())==0 or len(self.E12.get())==0:
	        # animation is not running; start it
	        messagebox.showwarning("Warning", "Please Fill All Input !")
	    else:
		    self.Run()
	...
	def save_animate_gif(self):
	    file_name = self.Aniname_gif.get()
	    

	    if len(self.Aniname_gif.get())==0:
	    	messagebox.showwarning("Warning", "Please Give Name File!")
	    else:
	    	animation = self.ani.save(file_name+'.gif', writer='imagemagick', fps=30)
	    	return animation

	def save_animate_mp4(self):
	    file_name = self.Aniname_mp4.get()
	    

	    if len(self.Aniname_mp4.get())==0:
	    	messagebox.showwarning("Warning", "Please Give Name File!")
	    else:
	    	animation = self.ani.save(file_name+'.mp4', fps=30)
	    	return animation
	...
	def init_window(self):
		...
		self.L0 = Label(self.frame, text='Parameter', font='Helvetica 12 bold', fg='red')
		...
		self.L0 = Label(self.frame, text='xleft').grid(row=2, column=0, sticky=W, padx= 22)
		...
		self.L0 = Label(self.frame, text='xright').grid(row=3, column=0, sticky=W, padx= 22)
		...
		self.B0 = Button(self.frame7,text='RUN',width=16,bg='green',command=self.on_click)
		self.B0.grid(row=14, padx=6,column=0)
